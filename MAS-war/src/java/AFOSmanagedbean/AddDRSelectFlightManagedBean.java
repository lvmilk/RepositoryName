...
@Named(value = "ADRSFMB")
@ViewScoped
public class AddDRSelectFlightManagedBean implements Serializable {
    ...
    public AddDRSelectFlightManagedBean() {
    }

    public void onDateChange() {
        if (dateSelected.after(new Date())) {
            FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_ERROR, "An error has occurred : Flight date cannot be after today", ""));
        } else {
    ...
    public void enterDebrief() throws Exception {
        selectedFi = fsb.findFlight(flightNo, dateString);
        if (fcb.hasDebriefingReport(selectedFi)) {
            FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_ERROR, "An error has occurred : Debriefing report has already been entered for this flight", ""));
        } else {
            FacesContext.getCurrentInstance().getExternalContext().getSessionMap().put("selectedFi", selectedFi);
